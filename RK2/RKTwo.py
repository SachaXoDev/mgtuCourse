...
class MusicPiece:
    ...
    def __repr__(self):
        return f"MusicPiece({self.piece_id}, '{self.title}', '{self.composer}', {self.duration}min, orch_id: {self.orchestra_id})"
...
def get_orchestra_pieces(orchestras, music_pieces):
    """Запрос 1: Список произведений с их оркестрами, отсортированный по названию оркестра."""
    result = []
...
def get_orchestra_total_durations(orchestras, music_pieces):
    """Запрос 2: Список оркестров с суммарной длительностью произведений."""
    orchestra_durations = {}
    for piece in music_pieces:
        orchestra_durations.setdefault(piece.orchestra_id, 0)
...
def get_orchestras_with_keyword(orchestras, music_pieces, orchestra_pieces, keyword='оркестр'):
    """Запрос 3: Список оркестров, содержащих keyword в названии, и их произведения."""
    filtered_orchestras = [o for o in orchestras if keyword.lower() in o.name.lower()]
...
def print_all_data(orchestras, music_pieces, orchestra_pieces):
...
if __name__ == "__main__":
    ...
    print_all_data(orchestras, music_pieces, orchestra_pieces)
    print("\n" + "=" * 60)

    print("\nЗАПРОС 1:")
    for piece, orchestra in get_orchestra_pieces(orchestras, music_pieces):
    ...
    print("\nЗАПРОС 2:")
    for orchestra, total in get_orchestra_total_durations(orchestras, music_pieces):
        print(f"Оркестр: {orchestra.name:<35} Суммарная длительность: {total}min")

    print("\nЗАПРОС 3:")
    for orchestra, pieces in get_orchestras_with_keyword(orchestras, music_pieces, orchestra_pieces):
